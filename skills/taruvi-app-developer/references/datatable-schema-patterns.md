...
Source of truth: <https://test-docs.taruvi.cloud/docs/data-service/> — `guides/migrations`, `/hierarchy`, `/graph-traversal`, `/relationships`, `/imports`.
...
| `required` | `NOT NULL` |
| `unique` | `UNIQUE` index |
| `minLength` / `maxLength` | `CHECK (length(field) >=/<= N)` |
