...
- **Analytics queries are the exception, not the default.** Use `appDataProvider` + `useCustom({ meta: { kind: "analytics" } })` only when the data provider can't express the query: **time-bucketed aggregations** (`date_trunc` / `extract` / `to_char`) and **parent rollups via FK traversal** both fail because `meta.groupBy` accepts only bare columns of the base resource (full rules + workarounds in `database-provider.md` → *Limits on `groupBy` and `aggregate` arguments*). Also: multi-table joins with grouped aggregation, window functions, recursive CTEs, external datasources. For permanent time / FK charts, prefer a denormalized bucket / parent column on the row; reach for analytics queries when the chart is shared or the table is large. Registering a query is a separate backend round-trip — avoid for anything `useList` + `meta.aggregate` + `meta.populate` can do.
